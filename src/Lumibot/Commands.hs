...
data SlackData = SlackData
  { teamId      :: String -- ^ ID for the `team` this is being sent from
  , channelId   :: String -- ^ ID for the channel this is being sent from
  , channelName :: String -- ^ Name for the channel this is being sent from
  , timestamp   :: String -- ^ Unix timestamp for message
  , userId      :: String -- ^ ID for the user this is being sent from
  , userName    :: String -- ^ Name for the user this is being sent from
  , triggerWord :: String -- ^ The trigger word that was used to send this message
...
-- `CommandParser`
type CommandParser = Parsec String () Command
...
data SlackResponse = SuccessResponse String
                   | FailResponse String
...
commandWithoutTrigger :: String -> CommandParser
